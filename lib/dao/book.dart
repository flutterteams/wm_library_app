...
class BookDao {
  static getList(Store<WMState> store, {page = 1, pageSize = 10}) async {
    try {
      ...
      response = await dio.get(Config.BASE_URL + "/api/bookList",
          queryParameters: {
...
//      print(response.data);
  ...
  static getId(Store<WMState> store, String id) async {
    try {
      ...
      response = await dio.get(Config.BASE_URL + "/api/bookDetails",
          queryParameters: {"bookId": id.toString()});
  ...
  static addBook(Store<WMState> store) async {
    try {
      ...
      response = await dio.post(Config.BASE_URL + "/api/admin/addbook", data: {
        "type": book.type_id,
        "title": book.title,
        "author": book.author,
  ...
  static editBook(Store<WMState> store) async {
    try {
      ...
      Book book = store.state.book;
      response =
          await dio.post(Config.BASE_URL + "/api/admin/bookDetails", data: {
        "bookId": book.id,
        "typeId": book.type_id,
        "title": book.title,
        "author": book.author,
  ...
  static delBook(Store<WMState> store, int id) async {
    try {
      ...
      List bookList = store.state.bookMap.containsKey('list')
          ? store.state.bookMap['list']
          : [];

      response =
          await dio.post(Config.BASE_URL + "/api/admin/offlineBook", data: {
        "bookId": id,
      });
      print(response.data);
      if (response.data['code'] == 1) {
